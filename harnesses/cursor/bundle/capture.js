...
function resolveCaptureConfig(cwd, log6) {
  ...
  if (!resolved.collect) {
    log6(`capture disabled for cwd=${effCwd} via ${resolved.found?.path}`);
    return null;
  }
...
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path = join12(hooksDir, filename);
  return {
    path,
    log(msg) {
      try {
...
// dist/src/hooks/cursor/spawn-wiki-worker.js
var HOME = homedir11();
var wikiLogger = makeWikiLogger(join15(HOME, ".cursor", "hooks"));
...
function spawnCursorWikiWorker(opts) {
  ...
  writeFileSync6(configFile, JSON.stringify({
    ...
    wikiLog: WIKI_LOG,
    hooksDir: join15(HOME, ".cursor", "hooks"),
...
function resolveEmbedDaemonPath() {
  return join24(dirname9(fileURLToPath3(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
async function main() {
  ...
  const config = resolveCaptureConfig(resolveCwd(input), log5);
  if (!config)
    return;
