...
var MEMORY_PATH = join11(homedir8(), ".deeplake", "memory");
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join14(homedir9(), ".hivemind", "graphs");
}
...
function privateStoreRoot() {
  return process.env.HIVEMIND_DOCS_PRIVATE_DIR || join18(homedir10(), ".hivemind", "docs-private");
}
function storeFile(project, scope) {
  const key = createHash5("sha256").update(`${project}\0${scope}`).digest("hex");
  return join18(privateStoreRoot(), `${key}.json`);
}
...
${firstDocLine(h.content)}`);
  ...
  if (path === "" || path === "index.md")
    ...
    dir = path.slice(0, -"/index.md".length);
...
// dist/src/docs/docs-command.js
var DOCS_ROOT = "/docs";
var DOCS_PREFIX = "/docs/";
async function tryDocsRead(rewrittenCommand, query, docsTable, opts = {}) {
  const ls = rewrittenCommand.replace(/\s+2>\S+/g, "").trim().match(/^ls\s+(?:-\S+\s+)*(\S+)\s*$/);
  if (ls) {
    const dir = stripQuotes(ls[1]).replace(/\/+$/, "") || "/";
    if (dir !== DOCS_ROOT)
  ...
  if (hasTraversal(virtualPath))
    return null;
  const normalized = virtualPath.replace(/\/+$/, "") || "/";
  if (normalized !== DOCS_ROOT && !virtualPath.startsWith(DOCS_PREFIX))
    return null;
...
function resolveEmbedDaemonPath() {
  return join19(dirname10(fileURLToPath2(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
__HIVEMIND_RESULT__`;
  ...
  try {
    ...
    log5(`docs vfs failed: ${err.message}`);
    docsBody = "(docs temporarily unavailable \u2014 try again)";
  }
  if (docsBody !== null) {
    log5(`docs vfs intercept: ${command.slice(0, 80)}`);
    const echoCmd = `cat <<'__HIVEMIND_RESULT__'
${docsBody}
__HIVEMIND_RESULT__`;
    process.stdout.write(JSON.stringify({
