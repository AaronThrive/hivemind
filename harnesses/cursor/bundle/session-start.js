...
var LOCAL_MANIFEST_PATH = join8(homedir6(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join8(homedir6(), ".claude", "hivemind", "local-mined.lock");
...
var HOME = homedir7();
var HIVEMIND_DIR = join9(HOME, ".claude", "hivemind");
var LOG_PATH = join9(HOME, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join9(HOME, ".claude", "projects");
var LOCK_STALE_MS2 = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname4(fileURLToPath(import.meta.url));
    const cliPath = join9(thisDir, "..", "..", "bundle", "cli.js");
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync(join9(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  ...
  try {
    mkdirSync7(join9(HOME, ".claude", "hooks"), { recursive: true });
...
import { existsSync as existsSync6, mkdirSync as mkdirSync8, readFileSync as readFileSync9, readdirSync as readdirSync2, statSync as statSync3, writeFileSync as writeFileSync6 } from "node:fs";
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync8(join15(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync9(join16(home, ".codex"));
  const piInstalled = existsSync9(join16(home, ".pi", "agent"));
  const hermesInstalled = existsSync9(join16(home, ".hermes"));
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    if (staleDir && staleDir !== dirName && !(opts.dryRun ?? false) && existsSync10(skillFile) && existsSync10(join17(root, staleDir))) {
      try {
...
function lastBuildPath(baseDir, worktreeId) {
  if (worktreeId !== void 0) {
    return join20(baseDir, "worktrees", worktreeId, ".last-build.json");
  }
  return join20(baseDir, ".last-build.json");
}
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join22(homedir12(), ".hivemind", "graphs");
}
function repoDir(repoKey) {
  return join22(graphsRoot(), repoKey);
}
...
function graphContextLine(cwd, deps = {}) {
  ...
  try {
    ...
    baseDir = repoDir(key);
    snapshotsDir = join23(baseDir, "snapshots");
  } catch {
    return null;
  }
  ...
  const snapshotPath = join23(snapshotsDir, `${snapshotFile}.json`);
...
// dist/src/hooks/cursor/session-start.js
var log7 = (msg) => log("cursor-session-start", msg);
var __bundleDir = dirname12(fileURLToPath2(import.meta.url));
    ...
    try {
      ...
      if (config) {
        ...
        if (collectHere) {
          ...
          log7(dirRes && !dirRes.collect ? `placeholder + schema ensure skipped (.hivemind collect:false ${dirRes.found?.path})` : "placeholder + schema ensure skipped (HIVEMIND_CAPTURE=false)");
        }
        const known = await api.knownTablesOrNull();
