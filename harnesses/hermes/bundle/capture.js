...
function resolveCaptureConfig(cwd, log7) {
  ...
  if (!resolved.collect) {
    log7(`capture disabled for cwd=${effCwd} via ${resolved.found?.path}`);
    return null;
  }
...
import { openSync as openSync2, closeSync as closeSync2, writeSync, unlinkSync as unlinkSync3, existsSync as existsSync3, readFileSync as readFileSync6 } from "node:fs";
...
import { existsSync as existsSync4, mkdirSync as mkdirSync5, readFileSync as readFileSync7, renameSync as renameSync2, writeFileSync as writeFileSync4 } from "node:fs";
...
function ensurePluginNodeModulesLink(opts) {
  ...
  const target = opts.sharedNodeModules ?? join10(homedir8(), ".hivemind", "embed-deps", "node_modules");
  const pluginDir = dirname4(opts.bundleDir);
  const link = join10(pluginDir, "node_modules");
...
import { readFileSync as readFileSync8, writeFileSync as writeFileSync5, writeSync as writeSync2, mkdirSync as mkdirSync7, existsSync as existsSync6, unlinkSync as unlinkSync4, openSync as openSync3, closeSync as closeSync3, statSync as statSync3 } from "node:fs";
import { homedir as homedir9 } from "node:os";
import { join as join11 } from "node:path";
var dlog = (msg) => log("summary-state", msg);
var STATE_DIR = join11(homedir9(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function statePath(sessionId) {
  return join11(STATE_DIR, `${sessionId}.json`);
}
function lockPath2(sessionId) {
  return join11(STATE_DIR, `${sessionId}.lock`);
}
...
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path2 = join12(hooksDir, filename);
  return {
    path: path2,
    log(msg) {
      try {
...
function getInstalledVersion(bundleDir, pluginManifestDir) {
  try {
    const pluginJson = join13(bundleDir, "..", pluginManifestDir, "plugin.json");
    const plugin = JSON.parse(readFileSync9(pluginJson, "utf-8"));
    if (plugin.version)
      return plugin.version;
  } catch {
  }
  try {
    const stamp = readFileSync9(join13(bundleDir, "..", ".hivemind_version"), "utf-8").trim();
    if (stamp)
      return stamp;
  } catch {
  }
...
function resolveCliBin(cli, fallback) {
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join14(homedir10(), ".claude", "local", cli);
  return isWin ? `${local}.cmd` : local;
}

// dist/src/hooks/hermes/spawn-wiki-worker.js
var HOME = homedir11();
var wikiLogger = makeWikiLogger(join15(HOME, ".hermes", "hooks"));
...
function spawnHermesWikiWorker(opts) {
  ...
  const tmpDir = join15(tmpdir2(), `deeplake-wiki-${sessionId}-${Date.now()}`);
  ...
  writeFileSync6(configFile, JSON.stringify({
    ...
    wikiLog: WIKI_LOG,
    hooksDir: join15(HOME, ".hermes", "hooks"),
    promptTemplate: WIKI_PROMPT_TEMPLATE
  }));
  wikiLog(`${reason}: spawning summary worker for ${sessionId}`);
  const workerPath = join15(bundleDir, "wiki-worker.js");
...
import { appendFileSync as appendFileSync3, mkdirSync as mkdirSync10, readFileSync as readFileSync10, readdirSync, statSync as statSync4, unlinkSync as unlinkSync5 } from "node:fs";
import { homedir as homedir12 } from "node:os";
import { join as join16 } from "node:path";
var dlog2 = (msg) => log("session-event-cache", msg);
var CACHE_DIR = join16(homedir12(), ".claude", "hooks", "session-cache");
...
import { writeFileSync as writeFileSync7, mkdirSync as mkdirSync11, appendFileSync as appendFileSync4, chmodSync } from "node:fs";
  ...
  switch (agent) {
    ...
    case "claude_code":
      return firstExistingPath([
        join17(home, ".claude", "local", "claude"),
    ...
    case "codex":
      return firstExistingPath([
        "/usr/local/bin/codex",
        "/usr/bin/codex",
        join17(home, ".npm-global", "bin", "codex"),
        join17(home, ".local", "bin", "codex"),
    ...
    case "cursor":
      return firstExistingPath([
        ...
        join17(home, ".npm-global", "bin", "cursor-agent"),
        join17(home, ".local", "bin", "cursor-agent"),
        "/opt/homebrew/bin/cursor-agent"
      ]) ?? "/usr/local/bin/cursor-agent";
    case "hermes":
      return firstExistingPath([
        join17(home, ".local", "bin", "hermes"),
        "/usr/local/bin/hermes",
        "/usr/bin/hermes",
        join17(home, ".npm-global", "bin", "hermes"),
        "/opt/homebrew/bin/hermes"
      ]) ?? join17(home, ".local", "bin", "hermes");
    case "pi":
      return firstExistingPath([
        join17(home, ".local", "bin", "pi"),
        "/usr/local/bin/pi",
        "/usr/bin/pi",
        join17(home, ".npm-global", "bin", "pi"),
        "/opt/homebrew/bin/pi"
      ]) ?? join17(home, ".local", "bin", "pi");
  }
}
...
// dist/src/skillify/spawn-skillify-worker.js
var HOME2 = homedir14();
var SKILLIFY_LOG = join18(HOME2, ".claude", "hooks", "skillify.log");
function skillifyLog(msg) {
  try {
    mkdirSync11(dirname7(SKILLIFY_LOG), { recursive: true });
...
function spawnSkillifyWorker(opts) {
  ...
  const tmpDir = join18(tmpdir3(), `deeplake-skillify-${projectKey}-${Date.now()}`);
  ...
  const workerPath = join18(bundleDir, "skillify-worker.js");
  spawnDetachedNodeWorker(workerPath, [configFile]);
  skillifyLog(`${reason}: spawned skillify worker for ${projectKey}`);
}

// dist/src/skillify/state.js
import { readFileSync as readFileSync11, writeFileSync as writeFileSync8, writeSync as writeSync3, mkdirSync as mkdirSync12, renameSync as renameSync5, rmdirSync, existsSync as existsSync9, lstatSync as lstatSync2, unlinkSync as unlinkSync6, openSync as openSync4, closeSync as closeSync4 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join19(homedir15(), ".deeplake", "state", "skillify");
}
...
function migrateLegacyStateDir() {
  ...
  const legacy = join20(dirname8(current), "skilify");
...
function statePath2(projectKey) {
  return join21(getStateDir(), `${projectKey}.json`);
}
function lockPath3(projectKey) {
  return join21(getStateDir(), `${projectKey}.lock`);
}
...
function tryAcquireWorkerLock(projectKey, maxAgeMs = 10 * 60 * 1e3) {
  ...
  if (existsSync9(p)) {
    try {
      const ageMs = Date.now() - parseInt(readFileSync11(p, "utf-8"), 10);
      if (Number.isFinite(ageMs) && ageMs < maxAgeMs)
        return false;
    } catch (readErr) {
...
import { existsSync as existsSync10, mkdirSync as mkdirSync13, readFileSync as readFileSync12, writeFileSync as writeFileSync9 } from "node:fs";
...
function loadScopeConfig() {
  ...
  if (!existsSync10(CONFIG_PATH))
    return DEFAULT;
  try {
    const raw = JSON.parse(readFileSync12(CONFIG_PATH, "utf-8"));
...
function isHivemindPluginEnabled() {
  try {
    ...
    const settings = JSON.parse(readFileSync13(settingsPath, "utf-8"));
...
import { existsSync as existsSync11, lstatSync as lstatSync3, mkdirSync as mkdirSync14, readFileSync as readFileSync14, renameSync as renameSync6, unlinkSync as unlinkSync7, writeFileSync as writeFileSync10 } from "node:fs";
...
function resolveEmbedDaemonPath() {
  return join25(dirname10(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
async function main() {
  ...
  const config = resolveCaptureConfig(input.cwd ?? process.cwd(), log6);
  if (!config)
    return;
