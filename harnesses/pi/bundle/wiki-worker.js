...
async function main() {
  try {
    ...
    if (existsSync4(tmpSummary)) {
      ...
      if (!execSucceeded) {
        wlog(summaryChanged ? "pi --print failed after a partial summary write; skipping upload to avoid advancing the offset" : "pi --print failed without producing a new summary; skipping upload");
        return;
      }
      if (raw.trim()) {
