...
async function main() {
  try {
    ...
    const likePat = esc2(`/sessions/%${cfg.sessionId}%`);
    ...
    const fetched = await query(`SELECT message, creation_date FROM "${cfg.sessionsTable}" WHERE path LIKE E'${likePat}' ORDER BY creation_date DESC LIMIT ${WIKI_FALLBACK_MAX_ROWS}`);
    ...
    if (existsSync4(tmpSummary)) {
      ...
      if (!execSucceeded) {
        wlog(summaryChanged ? "codex exec failed after a partial summary write; skipping upload to avoid advancing the offset" : "codex exec failed without producing a new summary; skipping upload");
        return;
      }
      if (raw.trim()) {
