...
function getIndexMarkerDir() {
  return process.env.HIVEMIND_INDEX_MARKER_DIR ?? join10(tmpdir(), "hivemind-deeplake-indexes");
}
function buildIndexMarkerPath(workspaceId, orgId, table, suffix) {
  const markerKey = [workspaceId, orgId, table, suffix].join("__").replace(/[^a-zA-Z0-9_.-]/g, "_");
  return join10(getIndexMarkerDir(), `${markerKey}.json`);
}
...
import { readFileSync as readFileSync7, writeFileSync as writeFileSync4, mkdirSync as mkdirSync6, openSync as openSync2, closeSync as closeSync2, unlinkSync as unlinkSync3, statSync as statSync2 } from "node:fs";
...
function queuePath() {
  return join9(homedir7(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
function writeQueue(q) {
  ...
  mkdirSync6(join9(home, ".deeplake"), { recursive: true, mode: 448 });
...
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync6(join9(homedir7(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
import { existsSync as existsSync5, mkdirSync as mkdirSync8, readFileSync as readFileSync9, readdirSync as readdirSync2, statSync as statSync3, writeFileSync as writeFileSync6 } from "node:fs";
...
function migrateLegacyStateDir() {
  ...
  const legacy = join13(dirname6(current), "skilify");
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync7(join14(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync8(join15(home, ".codex"));
  const piInstalled = existsSync8(join15(home, ".pi", "agent"));
  const hermesInstalled = existsSync8(join15(home, ".hermes"));
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    const staleFile = staleOwned ? join16(root, staleDir, "SKILL.md") : null;
    ...
    if (staleDir && staleDir !== dirName && !(opts.dryRun ?? false) && existsSync9(skillFile) && existsSync9(join16(root, staleDir))) {
      try {
        ...
        if (canonicalRecorded && staleEntry) {
          rmSync(join16(root, staleDir), { recursive: true, force: true });
...
function retireEntry(entry) {
  ...
  rmSync2(join17(entry.installRoot, entry.dirName), { recursive: true, force: true });
  removePullEntry(entry.install, entry.installRoot, entry.dirName);
}
function migrateEntry(entry) {
  if (entry.install !== "global")
    return false;
  const staleDir = join17(entry.installRoot, entry.dirName);
  const staleFile = join17(staleDir, "SKILL.md");
  ...
  const cappedDirPath = join17(entry.installRoot, cappedDir);
  const cappedFile = join17(cappedDirPath, "SKILL.md");
...
function spawnGraphPullWorker(cwd, bundleDir, deps = {}) {
  if (process.env.HIVEMIND_GRAPH_PULL === "0")
    return;
  const workerPath = join18(bundleDir, "graph-pull-worker.js");
  const opts = {
    detached: true,
...
// dist/src/hooks/codex/session-start.js
var log6 = (msg) => log("codex-session-start", msg);
var __bundleDir = dirname9(fileURLToPath2(import.meta.url));
async function main() {
  ...
  if (!creds?.token) {
    log6("no credentials found \u2014 run auth login to authenticate");
  ...
  {
    const setupScript = join19(__bundleDir, "session-start-setup.js");
    ...
    child.stdin?.end();
    child.unref();
    log6("spawned async setup process");
  }
