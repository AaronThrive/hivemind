...
function getInstalledVersion(bundleDir, pluginManifestDir) {
  ...
  try {
    const stamp = readFileSync7(join10(bundleDir, "..", ".hivemind_version"), "utf-8").trim();
    if (stamp)
      return stamp;
  } catch {
  }
...
var { log: wikiLog } = makeWikiLogger(join11(homedir6(), ".codex", "hooks"));
...
async function createPlaceholder(api, table, sessionId, cwd, userName, orgName, workspaceId) {
  await createPlaceholderSummary((sql) => api.query(sql), { table, sessionId, cwd, userName, orgName, workspaceId, agent: "codex", pluginVersion: PLUGIN_VERSION }, wikiLog);
}
async function main() {
  if (process.env.HIVEMIND_WIKI_WORKER === "1")
    return;
  const input = await readStdin();
  spawnDetachedNodeWorker(join11(__bundleDir, "graph-deps-worker.js"));
    ...
    try {
      ...
      if (base) {
        ...
        if (captureEnabled && dirRes.collect) {
          ...
          log5(!dirRes.collect ? `setup skipped \u2014 .hivemind collect:false (${dirRes.found?.path})` : "setup skipped \u2014 HIVEMIND_CAPTURE=false");
        }
      }
    } catch (e) {
