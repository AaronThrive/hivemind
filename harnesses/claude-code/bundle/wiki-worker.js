...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync3, existsSync as existsSync4, appendFileSync as appendFileSync3, mkdirSync as mkdirSync5, rmSync } from "node:fs";
...
import { appendFileSync as appendFileSync2, mkdirSync as mkdirSync3, readFileSync as readFileSync2, readdirSync, statSync as statSync2, unlinkSync as unlinkSync2 } from "node:fs";
import { homedir as homedir4 } from "node:os";
import { join as join4 } from "node:path";
var CACHE_DIR = join4(homedir4(), ".claude", "hooks", "session-cache");
...
function sessionEventCachePath(sessionId) {
  return join4(CACHE_DIR, `${sessionId}.jsonl`);
}
function readSessionEventCache(sessionId) {
  if (!sessionId || sessionEventCacheDisabled())
    return null;
  try {
    const raw = readFileSync2(sessionEventCachePath(sessionId), "utf-8");
    return raw.split("\n").filter((l) => l.length > 0);
  } catch {
    return null;
  }
...
var WIKI_FALLBACK_MAX_ROWS = 2e3;
function newRowsFromWindow(window, total, prevOffset) {
  if (prevOffset <= 0)
    return window.slice();
  const newCount = Math.max(0, total - prevOffset);
  if (newCount >= window.length)
    return window.slice();
...
import { openSync as openSync2, closeSync as closeSync2, writeSync as writeSync2, unlinkSync as unlinkSync3, existsSync as existsSync2, readFileSync as readFileSync3 } from "node:fs";
...
// dist/src/embeddings/client.js
var SHARED_DAEMON_PATH = join5(homedir5(), ".hivemind", "embed-deps", "embed-daemon.js");
...
async function embedSummaryWithWarmup(text, kind = "document", opts = {}) {
  ...
  try {
    ...
    if (!warm)
      log3("embed-summary: daemon warmup did not confirm ready; attempting embed anyway");
    let vec = await client.embed(text, kind);
    if (vec && vec.length > 0)
      return vec;
    log3("embed-summary: first attempt returned no vector; retrying once");
    vec = await client.embed(text, kind);
    return vec && vec.length > 0 ? vec : null;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    log3(`embed-summary: failed, writing NULL embedding: ${msg}`);
    return null;
  }
}
...
import { existsSync as existsSync3, mkdirSync as mkdirSync4, readFileSync as readFileSync4, renameSync, writeFileSync as writeFileSync2 } from "node:fs";
import { homedir as homedir6 } from "node:os";
import { dirname as dirname2, join as join6 } from "node:path";
var _configPath = () => process.env.HIVEMIND_CONFIG_PATH ?? join6(homedir6(), ".deeplake", "config.json");
var _cache = null;
var _migrated = false;
...
function defaultResolveTransformers() {
  const sharedDir = join7(homedir7(), ".hivemind", "embed-deps");
  try {
...
var tmpJsonl = join8(tmpDir, "session.jsonl");
var tmpSummary = join8(tmpDir, "summary.md");
function wlog(msg) {
  try {
    mkdirSync5(cfg.hooksDir, { recursive: true });
...
var EVENT_FETCH_RETRIES = parseNonNegativeInt(process.env.HIVEMIND_WIKI_EVENT_RETRIES, 5);
var EVENT_FETCH_BACKOFF_MS = parseNonNegativeInt(process.env.HIVEMIND_WIKI_EVENT_BACKOFF_MS, 1500);
...
async function main() {
  try {
    const dbFetch = async () => {
      const like = esc2(`/sessions/%${cfg.sessionId}%`);
      const cnt = await query(`SELECT count(*) AS n FROM "${cfg.sessionsTable}" WHERE path LIKE '${like}'`);
      const total = Number(cnt[0]?.["n"] ?? 0);
      if (total === 0)
        return { rows: [], total: 0 };
      const r = await query(`SELECT message, creation_date FROM "${cfg.sessionsTable}" WHERE path LIKE '${like}' ORDER BY creation_date DESC LIMIT ${WIKI_FALLBACK_MAX_ROWS}`);
      return { rows: r.reverse(), total };
    };
    const dbFetchWithRetry = async () => {
      let f = await dbFetch();
    ...
    if (rows.length === 0) {
      ...
      try {
        await query(`DELETE FROM "${cfg.memoryTable}" WHERE path = '${esc2(`/summaries/${cfg.userName}/${cfg.sessionId}.md`)}' AND description = 'in progress'`);
      } catch (e) {
    ...
    let jsonlServerPath;
    if (usedLocalCache) {
      jsonlServerPath = buildSessionPath({ userName: cfg.userName, orgName: cfg.orgName, workspaceId: cfg.workspaceId }, cfg.sessionId);
    } else {
      ...
      jsonlServerPath = pathRows.length > 0 ? pathRows[0].path : `/sessions/unknown/${cfg.sessionId}.jsonl`;
    }
    let prevOffset = 0;
    ...
    const newRows = usedLocalCache ? prevOffset > 0 ? rows.slice(prevOffset) : rows : newRowsFromWindow(rows, dbTotal, prevOffset);
    ...
    const summaryBeforeExec = existsSync4(tmpSummary) ? readFileSync5(tmpSummary, "utf-8") : null;
    try {
    ...
    if (existsSync4(tmpSummary)) {
      const raw = readFileSync5(tmpSummary, "utf-8");
      const summaryChanged = summaryBeforeExec === null ? raw.trim().length > 0 : raw !== summaryBeforeExec;
      if (!execSucceeded) {
        wlog(summaryChanged ? "claude -p failed after a partial summary write; skipping upload to avoid advancing the offset" : "claude -p failed without producing a new summary; skipping upload");
        return;
      }
      if (raw.trim()) {
        ...
        if (!embeddingsDisabled()) {
          const daemonEntry = join8(dirname3(fileURLToPath(import.meta.url)), "embeddings", "embed-daemon.js");
