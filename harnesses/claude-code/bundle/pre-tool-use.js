...
import { join as join25, dirname as dirname13, sep } from "node:path";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join4(homedir3(), ".deeplake", "state", "skillify");
}
...
import { existsSync as existsSync3, lstatSync, mkdirSync as mkdirSync2, readFileSync as readFileSync3, renameSync as renameSync2, unlinkSync, writeFileSync } from "node:fs";
...
function migrateLegacyStateDir() {
  ...
  const legacy = join5(dirname3(current), "skilify");
...
function manifestPath() {
  return join6(getStateDir(), "pulled.json");
}
...
import { readFileSync as readFileSync4, writeFileSync as writeFileSync2, mkdirSync as mkdirSync3, openSync, closeSync, unlinkSync as unlinkSync2, statSync } from "node:fs";
...
function queuePath() {
  return join7(homedir4(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
async function withQueueLock(fn) {
  ...
  mkdirSync3(join7(homedir4(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync3, mkdirSync as mkdirSync4, unlinkSync as unlinkSync3, renameSync as renameSync3 } from "node:fs";
import { join as join8 } from "node:path";
import { homedir as homedir5 } from "node:os";
function configDir() {
  return join8(homedir5(), ".deeplake");
}
function credsPath() {
  return join8(configDir(), "credentials.json");
}
function loadCredentials(readFile = (p) => readFileSync5(p, "utf-8")) {
  try {
...
function emptySessionBodyNotice(rowCount) {
  const plural = rowCount === 1 ? "row" : "rows";
  return `[hivemind: this session has ${rowCount} stored ${plural} but the message body is empty \u2014 capture recorded metadata (size only) without content, so the transcript body is unavailable. This is not an empty file; there is nothing to read here.]`;
}
...
import { openSync as openSync2, closeSync as closeSync2, writeSync, unlinkSync as unlinkSync4, existsSync as existsSync5, readFileSync as readFileSync7 } from "node:fs";
...
import { existsSync as existsSync6, mkdirSync as mkdirSync6, readFileSync as readFileSync8, renameSync as renameSync4, writeFileSync as writeFileSync5 } from "node:fs";
import { homedir as homedir7 } from "node:os";
import { dirname as dirname5, join as join11 } from "node:path";
var _configPath = () => process.env.HIVEMIND_CONFIG_PATH ?? join11(homedir7(), ".deeplake", "config.json");
var _cache = null;
var _migrated = false;
...
function defaultResolveTransformers() {
  const sharedDir = join12(homedir8(), ".hivemind", "embed-deps");
  try {
...
function resolveDaemonPath() {
  return join13(dirname6(fileURLToPath3(import.meta.url)), "..", "embeddings", "embed-daemon.js");
}
...
import { existsSync as existsSync9, mkdirSync as mkdirSync10, readFileSync as readFileSync11, renameSync as renameSync7, writeFileSync as writeFileSync8 } from "node:fs";
import { createHash as createHash3 } from "node:crypto";
import { join as join17, dirname as dirname10 } from "node:path";

// dist/src/graph/last-build.js
import { existsSync as existsSync7, mkdirSync as mkdirSync7, readFileSync as readFileSync9, renameSync as renameSync5, writeFileSync as writeFileSync6 } from "node:fs";
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join16(homedir9(), ".hivemind", "graphs");
}
function repoDir(repoKey) {
  return join16(graphsRoot(), repoKey);
}
...
function loadSnapshotOrError(cwd, fn) {
  ...
  const snapPath = join17(baseDir, "snapshots", `${fileBase}.json`);
  if (!existsSync9(snapPath)) {
    return { kind: "no-graph", message: `Snapshot file missing on disk: ${snapPath}` };
  }
  let snap;
  try {
    snap = JSON.parse(readFileSync11(snapPath, "utf8"));
  } catch (e) {
...
function renderIndex(snap, baseDir, cwd) {
  ...
  lines.push(`Source:  ${join17(baseDir, "snapshots", `${commit ? snap.graph.commit_sha : "?"}.json`)}`);
  lines.push("");
...
function handlesPath(baseDir, worktreeId) {
  return join17(baseDir, "worktrees", worktreeId, ".find-handles.json");
}
...
function pickByScopePrecedence(rows, readerScope) {
  ...
  for (const r of rows) {
    ...
    const rank2 = s === readerScope ? 2 : s === MAIN_SCOPE ? 1 : 0;
    if (rank2 === 0)
      continue;
    if (best === null || rank2 > bestRank || rank2 === bestRank && r.version > best.version) {
      best = r;
      bestRank = rank2;
    }
  }
...
function computeFingerprintAt(git, ref, files) {
  ...
  for (const line of out.split("\n")) {
    ...
    const path2 = line.slice(tab + 1);
    if (meta.length >= 3 && meta[1] === "blob" && path2)
      fp[path2] = meta[2];
  }
  return fp;
}
...
import { readFileSync as readFileSync13, statSync as statSync2 } from "node:fs";
...
import { mkdirSync as mkdirSync11, readFileSync as readFileSync14, writeFileSync as writeFileSync9, renameSync as renameSync8, existsSync as existsSync10 } from "node:fs";
import { homedir as homedir10 } from "node:os";
import { createHash as createHash5 } from "node:crypto";
import { dirname as dirname11, join as join20 } from "node:path";
function privateStoreRoot() {
  return process.env.HIVEMIND_DOCS_PRIVATE_DIR || join20(homedir10(), ".hivemind", "docs-private");
}
function storeFile(project, scope) {
  const key = createHash5("sha256").update(`${project}\0${scope}`).digest("hex");
  return join20(privateStoreRoot(), `${key}.json`);
}
function readMap(file) {
  try {
    if (!existsSync10(file))
      return {};
    const raw = JSON.parse(readFileSync14(file, "utf-8"));
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
      return {};
    return raw;
  } catch {
...
function readPrivateDoc(project, scope, docId) {
  return readMap(storeFile(project, scope))[docId] ?? null;
}

// dist/src/docs/vfs-handler.js
async function handleDocsVfs(subpath, query, tableName, opts = {}) {
  const path2 = subpath.replace(/^\/+/, "").replace(/\/+$/, "");
  if (path2 === "find" || path2.startsWith("find/") && !path2.endsWith(".md")) {
    const q = path2 === "find" ? "" : path2.slice("find/".length).trim();
    if (q === "") {
    ...
    if (hits.length === 0)
      return { kind: "ok", body: `No docs match "${q}".` };
...
${firstDocLine(h.content)}`);
  ...
  let dir = null;
  if (path2 === "" || path2 === "index.md")
    dir = "";
  else if (path2.endsWith("/index.md"))
    dir = path2.slice(0, -"/index.md".length);
  else if (!path2.endsWith(".md"))
    dir = path2;
  if (dir !== null) {
    const meta = (await listDocMeta(query, tableName, { dirPrefix: dir, project: opts.project, readerScope: opts.readerScope })).map((r) => ({
      doc_id: r.doc_id,
      version: r.version,
    ...
    const directFiles = meta.filter((m) => m.status === "active" && dirOf(m.doc_id) === dir).map((m) => m.doc_id);
  ...
  const docId = path2.slice(0, -".md".length);
  if (opts.project !== void 0 && opts.readerScope && opts.readerScope !== MAIN_SCOPE) {
    const priv = readPrivateDoc(opts.project, opts.readerScope, docId);
    if (priv) {
      const banner2 = staleBanner(opts.git, priv.doc_id, priv.content, priv.source_fp);
      const header2 = `# ${priv.doc_id}
visibility: private (this branch, not pushed)  updated: ${priv.updated_at}
---
`;
      return { kind: "ok", body: header2 + banner2 + priv.content };
    }
  }
  const row = await getDocLatest(query, tableName, docId, { projectOrLegacy: opts.project, readerScope: opts.readerScope });
  if (!row)
    return { kind: "not-found", message: `${subpath}: No such file or directory` };
...
function staleBanner(git, docId, content, source_fp) {
  if (!git || !source_fp)
    return "";
  ...
  if (changed.length === 0)
    return "";
  return `> [!] This page may be stale \u2014 ${changed.length} source file(s) changed since it was written. Confirm against the source before relying on it: ${changed.join(", ")}
...
function resolveEmbedDaemonPath() {
  return join21(dirname12(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
async function readVirtualPathContents(api, memoryTable, sessionsTable, virtualPaths) {
  ...
  const sessionRowCounts = /* @__PURE__ */ new Map();
  for (const row of rows) {
    ...
    if (typeof path2 !== "string")
      continue;
    if (sourceOrder !== 0) {
      sessionRowCounts.set(path2, (sessionRowCounts.get(path2) ?? 0) + 1);
    }
    ...
    if (sourceOrder === 0) {
      ...
      const normalized = normalizeSessionPart(path2, content);
      if (!normalized)
        continue;
  ...
  for (const path2 of uniquePaths) {
    ...
    const rowCount = sessionRowCounts.get(path2) ?? 0;
    if (rowCount > 0) {
      result.set(path2, emptySessionBodyNotice(rowCount));
    }
  }
...
import { mkdirSync as mkdirSync12, readFileSync as readFileSync15, rmSync, writeFileSync as writeFileSync10 } from "node:fs";
import { join as join22 } from "node:path";
import { homedir as homedir11 } from "node:os";
var log5 = (msg) => log("query-cache", msg);
var DEFAULT_CACHE_ROOT = join22(homedir11(), ".deeplake", "query-cache");
var INDEX_CACHE_FILE = "index.md";
function getSessionQueryCacheDir(sessionId, deps = {}) {
  const { cacheRoot = DEFAULT_CACHE_ROOT } = deps;
  return join22(cacheRoot, sessionId);
}
function readCachedIndexContent(sessionId, deps = {}) {
  const { logFn = log5 } = deps;
  try {
    return readFileSync15(join22(getSessionQueryCacheDir(sessionId, deps), INDEX_CACHE_FILE), "utf-8");
  } catch (e) {
    if (e?.code === "ENOENT")
      return null;
...
function writeCachedIndexContent(sessionId, content, deps = {}) {
  ...
  try {
    ...
    writeFileSync10(join22(dir, INDEX_CACHE_FILE), content, "utf-8");
  } catch (e) {
...
var MEMORY_PATH = join23(homedir12(), ".deeplake", "memory");
...
import { readFileSync as readFileSync16, writeFileSync as writeFileSync11, writeSync as writeSync2, mkdirSync as mkdirSync13, existsSync as existsSync11, unlinkSync as unlinkSync5, openSync as openSync3, closeSync as closeSync3, statSync as statSync3 } from "node:fs";
import { homedir as homedir13 } from "node:os";
import { join as join24 } from "node:path";
var dlog2 = (msg) => log("summary-state", msg);
var STATE_DIR = join24(homedir13(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function ownerPath(sessionId) {
  return join24(STATE_DIR, `${sessionId}.owner`);
}
...
function recordSessionOwner(sessionId, agentComms = ["claude"], startPid = process.pid) {
  try {
    ...
    if (!owner)
      return;
    mkdirSync13(STATE_DIR, { recursive: true });
...
function ensureSessionOwner(sessionId, agentComms = ["claude"], startPid = process.pid) {
  if (existsSync11(ownerPath(sessionId)))
    return;
...
var __bundleDir = dirname13(fileURLToPath5(import.meta.url));
var READ_CACHE_ROOT = join25(homedir14(), ".deeplake", "query-cache");
function writeReadCacheFile(sessionId, virtualPath, content, deps = {}) {
  ...
  const expectedRoot = join25(cacheRoot, safeSessionId, "read");
  const absPath = join25(expectedRoot, rel);
  ...
  mkdirSync14(dirname13(absPath), { recursive: true });
  writeFileSync12(absPath, content, "utf-8");
  return absPath;
}
...
async function processPreToolUse(input, deps = {}) {
  ...
  const shellBundle = join25(__bundleDir, "shell", "deeplake-shell.js");
