...
import { existsSync as existsSync4, mkdirSync as mkdirSync7, readFileSync as readFileSync8, renameSync as renameSync2, writeFileSync as writeFileSync5 } from "node:fs";
...
async function autoUpdate(creds, opts) {
  ...
  if (isCodexManagedInstall(opts.bundleDir)) {
    log5(`agent=${opts.agent} skip: Codex-managed install (${opts.bundleDir}); updates come via marketplace snapshots (${Date.now() - t0}ms)`);
    return;
  }
  if (!creds?.token) {
...
var { log: wikiLog } = makeWikiLogger(join13(homedir9(), ".claude", "hooks"));
async function main() {
  if (process.env.HIVEMIND_WIKI_WORKER === "1")
    return;
  const input = await readStdin();
  spawnDetachedNodeWorker(join13(__bundleDir, "graph-deps-worker.js"));
  ...
  if (embeddingsDisabled()) {
    ...
    try {
      const daemonEntry = join13(__bundleDir, "embeddings", "embed-daemon.js");
