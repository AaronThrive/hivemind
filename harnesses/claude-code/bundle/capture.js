...
function findDirConfig(startDir, stopAt) {
  ...
  for (; ; ) {
    for (const name of DIR_CONFIG_FILENAMES) {
      ...
      try {
        const raw = parseDirConfig(readFileSync2(candidate, "utf-8"));
        if (raw)
...
// dist/src/hooks/shared/dir-gate.js
function resolveCaptureConfig(cwd, log8) {
  ...
  if (!resolved.collect) {
    log8(`capture disabled for cwd=${effCwd} via ${resolved.found?.path}`);
    return null;
  }
...
var LOG = join3(homedir2(), ".deeplake", "hook-debug.log");
function isDebug() {
...
function log(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync(dirname2(LOG), { recursive: true });
...
import { readFileSync as readFileSync3, writeFileSync, mkdirSync as mkdirSync2, openSync, closeSync, unlinkSync, statSync } from "node:fs";
...
function isPathInsideHome(path2, home) {
  const r = resolve2(path2);
...
function _isQueuePathInsideHome(path2, home) {
  return isPathInsideHome(path2, home);
}
...
import { readFileSync as readFileSync4, writeFileSync as writeFileSync2, mkdirSync as mkdirSync3, unlinkSync as unlinkSync2, renameSync } from "node:fs";
import { join as join5 } from "node:path";
import { homedir as homedir4 } from "node:os";
function configDir() {
  return join5(homedir4(), ".deeplake");
}
function credsPath() {
  return join5(configDir(), "credentials.json");
}
function loadCredentials(readFile = (p) => readFileSync4(p, "utf-8")) {
  try {
...
// dist/src/notifications/model-usage.js
import { closeSync as closeSync2, fstatSync, openSync as openSync2, readFileSync as readFileSync6, readSync } from "node:fs";
import { homedir as homedir5 } from "node:os";
import { join as join7 } from "node:path";
var log4 = (msg) => log("model-usage", msg);
function toNum(v) {
  return typeof v === "number" && Number.isSafeInteger(v) && v >= 0 ? v : void 0;
}
function assign(target, key, v) {
  const n = toNum(v);
  if (n !== void 0)
    target[key] = n;
}
function readWindowLines(fd, size, maxBytes) {
  if (size <= maxBytes) {
    const buf2 = Buffer.alloc(size);
    const n2 = size > 0 ? readSync(fd, buf2, 0, size, 0) : 0;
    return buf2.toString("utf-8", 0, n2).split("\n");
  }
...
function openTranscript(path2, window = 1048576) {
  if (!path2)
    return null;
  let fd;
  try {
    ...
    log4(`open failed (${path2}): ${e?.message ?? String(e)}`);
    return null;
  }
  let size;
  try {
    ...
    log4(`stat failed: ${e?.message ?? String(e)}`);
    return null;
  }
  const read = (maxBytes) => {
    try {
      ...
      log4(`read failed: ${e?.message ?? String(e)}`);
      return [];
    }
  };
  let wholeCache;
...
var CLAUDE_EFFORT_LEVELS = /* @__PURE__ */ new Set(["low", "medium", "high", "xhigh"]);
function readClaudeEffortLevel(cwd) {
  const candidates = [];
  if (cwd) {
    candidates.push(join7(cwd, ".claude", "settings.local.json"));
    candidates.push(join7(cwd, ".claude", "settings.json"));
  }
  candidates.push(join7(homedir5(), ".claude", "settings.json"));
  for (const p of candidates) {
    try {
      const j = JSON.parse(readFileSync6(p, "utf-8"));
      if (typeof j.effortLevel === "string") {
        const level = j.effortLevel.toLowerCase();
        if (CLAUDE_EFFORT_LEVELS.has(level))
          return level;
      }
    } catch {
    }
...
function claudeUsageExtra(u) {
  const extra = {};
  if (typeof u.service_tier === "string")
    extra.service_tier = u.service_tier;
  if (typeof u.speed === "string")
    extra.speed = u.speed;
  if (typeof u.inference_geo === "string")
    extra.inference_geo = u.inference_geo;
  const cc = u.cache_creation;
  if (cc && typeof cc === "object") {
    const ttl = {};
    const a = toNum(cc.ephemeral_1h_input_tokens);
    if (a !== void 0)
      ttl.ephemeral_1h = a;
    const b = toNum(cc.ephemeral_5m_input_tokens);
    if (b !== void 0)
      ttl.ephemeral_5m = b;
    if (Object.keys(ttl).length)
      extra.cache_creation_ttl = ttl;
  }
  const st = u.server_tool_use;
  if (st && typeof st === "object") {
    const s = {};
    const ws = toNum(st.web_search_requests);
    if (ws !== void 0)
      s.web_search_requests = ws;
    const wf = toNum(st.web_fetch_requests);
    if (wf !== void 0)
      s.web_fetch_requests = wf;
    if (Object.keys(s).length)
      extra.server_tool_use = s;
  }
  return Object.keys(extra).length > 0 ? extra : void 0;
}
function flattenClaudeText(content) {
  if (typeof content === "string")
    return content;
  if (!Array.isArray(content))
    return "";
  return content.map((b) => b && typeof b === "object" && b.type === "text" && typeof b.text === "string" ? b.text : "").join("");
}
var MIN_TRUNCATION_MATCH_LEN = 32;
function textMatches(entryText, expect) {
...
function scanClaudeTurnForCapture(transcriptPath, expectText, includeSidechain = false) {
  if (!transcriptPath)
    return { status: "retry" };
  ...
  if (!r)
    return { status: "retry" };
  try {
    return scanClaudeTurn(r.lines, expectText, includeSidechain) ?? scanClaudeTurn(r.readWhole(), expectText, includeSidechain) ?? { status: "retry" };
  } finally {
    r.close();
  }
}
function scanClaudeTurn(lines, expectText, includeSidechain) {
  if (!lines)
    return null;
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      if (!sawNonBlank) {
        log4("trailing partial record \u2014 retry");
        return { status: "retry" };
      }
      sawNonBlank = true;
      continue;
    ...
    if (entry.type !== "assistant" || !msg)
      continue;
    if (!includeSidechain && entry.isSidechain === true)
      continue;
    if (expectText !== void 0) {
      const text = flattenClaudeText(msg.content);
      if (!text.trim())
        continue;
      if (!textMatches(text, expectText)) {
        log4("newest assistant record does not match last_assistant_message \u2014 retry");
        return { status: "retry" };
      }
    }
    if (!msg.usage) {
      log4("newest assistant record has no usage \u2014 retry");
      return { status: "retry" };
    }
    const meta = {
      model: typeof msg.model === "string" ? msg.model : void 0,
      reasoning_effort: readClaudeEffortLevel(typeof entry.cwd === "string" ? entry.cwd : void 0) ?? null,
    ...
    if (extra)
      meta.usage_extra = extra;
    return { status: "match", meta };
  }
  return null;
}
async function parseClaudeTurnMetaLive(transcriptPath, expectText, includeSidechain = false, backoffMs = [25, 50, 100]) {
  ...
  if (scan.status !== "match") {
    log4(`turn meta unavailable after ${backoffMs.length + 1} reads \u2014 writing unenriched`);
    return null;
  }
...
import { readFileSync as readFileSync7, writeFileSync as writeFileSync4, writeSync, mkdirSync as mkdirSync5, existsSync as existsSync3, unlinkSync as unlinkSync3, openSync as openSync3, closeSync as closeSync3, statSync as statSync2 } from "node:fs";
import { homedir as homedir6 } from "node:os";
import { join as join8 } from "node:path";
var dlog = (msg) => log("summary-state", msg);
var STATE_DIR = join8(homedir6(), ".claude", "hooks", "summary-state");
...
function ownerPath(sessionId) {
  return join8(STATE_DIR, `${sessionId}.owner`);
}
function procInfo(pid) {
  try {
    const s = readFileSync7(`/proc/${pid}/stat`, "utf-8");
...
function recordSessionOwner(sessionId, agentComms = ["claude"], startPid = process.pid) {
  try {
    ...
    writeFileSync4(tmp, JSON.stringify(owner));
    renameAtomic(tmp, p);
  } catch (e) {
...
function ensureSessionOwner(sessionId, agentComms = ["claude"], startPid = process.pid) {
  if (existsSync3(ownerPath(sessionId)))
    return;
...
function markSummaryAttempt(sessionId) {
...
function retryBackoffMs(attemptsSinceSuccess) {
  if (attemptsSinceSuccess <= 0)
    return 0;
...
function shouldTrigger(state, cfg, now = Date.now()) {
  ...
  if (attempts > 0 && now - (state.lastAttemptAt ?? 0) < retryBackoffMs(attempts))
    return false;
...
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path2 = join9(hooksDir, filename);
  return {
    path: path2,
    log(msg) {
      try {
...
function getInstalledVersion(bundleDir, pluginManifestDir) {
  ...
  try {
    const stamp = readFileSync8(join10(bundleDir, "..", ".hivemind_version"), "utf-8").trim();
    if (stamp)
      return stamp;
  } catch {
  }
...
function resolveCliBin(cli, fallback) {
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join11(homedir7(), ".claude", "local", cli);
  return isWin ? `${local}.cmd` : local;
}

// dist/src/hooks/spawn-wiki-worker.js
var HOME = homedir8();
var wikiLogger = makeWikiLogger(join12(HOME, ".claude", "hooks"));
...
function spawnWikiWorker(opts) {
  ...
  writeFileSync5(configFile, JSON.stringify({
    ...
    wikiLog: WIKI_LOG,
    hooksDir: join12(HOME, ".claude", "hooks"),
...
// dist/src/hooks/session-event-cache.js
import { appendFileSync as appendFileSync3, mkdirSync as mkdirSync8, readFileSync as readFileSync9, readdirSync, statSync as statSync3, unlinkSync as unlinkSync4 } from "node:fs";
import { homedir as homedir9 } from "node:os";
import { join as join13 } from "node:path";
var dlog2 = (msg) => log("session-event-cache", msg);
var CACHE_DIR = join13(homedir9(), ".claude", "hooks", "session-cache");
function sessionEventCacheDisabled() {
  const v = process.env.HIVEMIND_SESSION_EVENT_CACHE;
  return v === "0" || v === "false";
}
function sessionEventCachePath(sessionId) {
  return join13(CACHE_DIR, `${sessionId}.jsonl`);
}
function appendSessionEvent(sessionId, line) {
  if (!sessionId || sessionEventCacheDisabled())
    return;
  try {
    mkdirSync8(CACHE_DIR, { recursive: true });
    appendFileSync3(sessionEventCachePath(sessionId), line + "\n");
  } catch (e) {
    dlog2(`append failed for ${sessionId}: ${e?.message ?? e}`);
  }
}
...
import { writeFileSync as writeFileSync6, mkdirSync as mkdirSync9, appendFileSync as appendFileSync4, chmodSync } from "node:fs";
...
import { readFileSync as readFileSync10, writeFileSync as writeFileSync7, writeSync as writeSync2, mkdirSync as mkdirSync10, renameSync as renameSync3, rmdirSync, existsSync as existsSync6, lstatSync, unlinkSync as unlinkSync5, openSync as openSync4, closeSync as closeSync4 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join16(homedir12(), ".deeplake", "state", "skillify");
}

// dist/src/skillify/legacy-migration.js
var dlog3 = (msg) => log("skillify-migrate", msg);
var attempted = false;
function migrateLegacyStateDir() {
  ...
  try {
    ...
    dlog3(`migrated ${legacy} -> ${current}`);
  } catch (err) {
    const code = err.code;
    if (code === "EXDEV" || code === "EPERM" || code === "ENOENT" || code === "EEXIST" || code === "ENOTEMPTY") {
      dlog3(`migration skipped (${code}); legacy dir left as-is or another process handled it`);
      return;
    }
    throw err;
  }
...
// dist/src/skillify/state.js
var dlog4 = (msg) => log("skillify-state", msg);
...
function withRmwLock2(projectKey, fn) {
  ...
  while (fd === null) {
    try {
      ...
      if (e.code !== "EEXIST")
        throw e;
      if (Date.now() > deadline) {
        dlog4(`rmw lock deadline exceeded for ${projectKey}, reclaiming stale lock`);
        try {
          unlinkSync5(rmw);
        } catch (unlinkErr) {
          dlog4(`stale rmw lock unlink failed for ${projectKey}: ${unlinkErr.message}`);
        }
        continue;
      }
  ...
  try {
    ...
    try {
      ...
      dlog4(`rmw lock cleanup failed for ${projectKey}: ${unlinkErr.message}`);
    }
  }
}
...
function tryAcquireWorkerLock(projectKey, maxAgeMs = 10 * 60 * 1e3) {
  ...
  if (existsSync6(p)) {
    try {
      const ageMs = Date.now() - parseInt(readFileSync10(p, "utf-8"), 10);
      if (Number.isFinite(ageMs) && ageMs < maxAgeMs)
        return false;
    } catch (readErr) {
      dlog4(`worker lock unreadable for ${projectKey}, treating as stale: ${readErr.message}`);
    }
    try {
      unlinkSync5(p);
    } catch (unlinkErr) {
      if (unlinkErr?.code !== "EISDIR" && unlinkErr?.code !== "EPERM" && unlinkErr?.code !== "ENOENT") {
        dlog4(`could not unlink stale worker lock for ${projectKey}: ${unlinkErr.message}`);
        return false;
      }
      let isDir = false;
      try {
      ...
      if (isDir) {
        try {
          ...
          dlog4(`rmdir stale lock skipped for ${projectKey}: ${rmErr.message}`);
        }
      }
    }
  }
...
import { existsSync as existsSync7, mkdirSync as mkdirSync11, readFileSync as readFileSync11, writeFileSync as writeFileSync8 } from "node:fs";
...
function loadScopeConfig() {
  ...
  if (!existsSync7(CONFIG_PATH))
    return DEFAULT;
  try {
    const raw = JSON.parse(readFileSync11(CONFIG_PATH, "utf-8"));
...
import { existsSync as existsSync8, lstatSync as lstatSync2, mkdirSync as mkdirSync12, readFileSync as readFileSync12, renameSync as renameSync4, unlinkSync as unlinkSync6, writeFileSync as writeFileSync9 } from "node:fs";
...
import { openSync as openSync5, closeSync as closeSync5, writeSync as writeSync3, unlinkSync as unlinkSync7, existsSync as existsSync9, readFileSync as readFileSync13 } from "node:fs";
...
// dist/src/embeddings/client.js
var SHARED_DAEMON_PATH = join21(homedir13(), ".hivemind", "embed-deps", "embed-daemon.js");
var log6 = (m) => log("embed-client", m);
function getUid() {
...
var EmbedClient = class {
  ...
  recycleDaemon(reportedPid) {
    ...
    if (pid === null) {
      try {
        pid = Number.parseInt(readFileSync13(this.pidPath, "utf-8").trim(), 10);
      } catch {
      }
    }
    ...
    try {
      unlinkSync7(this.socketPath);
    } catch {
    }
    try {
      unlinkSync7(this.pidPath);
    } catch {
    }
  }
  /**
  ...
  trySpawnDaemon() {
    let fd;
    try {
      fd = openSync5(this.pidPath, "wx", 384);
  ...
  isPidFileStale() {
    try {
      const raw = readFileSync13(this.pidPath, "utf-8").trim();
...
import { existsSync as existsSync10, mkdirSync as mkdirSync13, readFileSync as readFileSync14, renameSync as renameSync5, writeFileSync as writeFileSync10 } from "node:fs";
import { homedir as homedir14 } from "node:os";
import { dirname as dirname8, join as join22 } from "node:path";
var _configPath = () => process.env.HIVEMIND_CONFIG_PATH ?? join22(homedir14(), ".deeplake", "config.json");
var _cache = null;
var _migrated = false;
...
function writeUserConfig(patch) {
  ...
  writeFileSync10(tmp, JSON.stringify(merged, null, 2) + "\n", "utf-8");
...
function defaultResolveTransformers() {
  const sharedDir = join23(homedir15(), ".hivemind", "embed-deps");
  try {
...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join24(homedir16(), ".claude", "settings.json");
...
import { existsSync as existsSync11, lstatSync as lstatSync3, mkdirSync as mkdirSync14, readlinkSync, renameSync as renameSync6, rmSync, symlinkSync, statSync as statSync4 } from "node:fs";
import { homedir as homedir17 } from "node:os";
import { basename as basename3, dirname as dirname9, join as join25 } from "node:path";
function ensurePluginNodeModulesLink(opts) {
  if (basename3(opts.bundleDir) !== "bundle") {
    return { kind: "not-bundle-layout", bundleDir: opts.bundleDir };
  }
  const target = opts.sharedNodeModules ?? join25(homedir17(), ".hivemind", "embed-deps", "node_modules");
  const pluginDir = dirname9(opts.bundleDir);
  const link = join25(pluginDir, "node_modules");
  if (!existsSync11(target)) {
    return { kind: "shared-deps-missing", target };
  }
  let linkStat;
  try {
    linkStat = lstatSync3(link);
  } catch {
...
// dist/src/hooks/capture.js
var log7 = (msg) => log("capture", msg);
function resolveEmbedDaemonPath() {
  return join26(dirname10(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
async function main() {
  ...
  if (!isHivemindPluginEnabled()) {
    log7("plugin disabled, skipping capture");
    return;
  }
  if (!entrypointPassesOnlyCliGate())
    return;
  const input = await readStdin();
  const config = resolveCaptureConfig(input.cwd ?? process.cwd(), log7);
  if (!config)
    return;
  ...
  if (input.prompt !== void 0) {
    log7(`user session=${input.session_id}`);
    entry = {
    ...
    log7(`tool=${input.tool_name} session=${input.session_id}`);
    entry = {
      id: crypto.randomUUID(),
      ...meta,
      type: "tool_call",
    ...
    log7(`assistant session=${input.session_id}`);
    const expectText = typeof input.last_assistant_message === "string" ? input.last_assistant_message.trim() : "";
    if (!expectText)
      log7("empty last_assistant_message \u2014 skipping turn-meta enrichment");
    const modelMeta = expectText ? await parseClaudeTurnMetaLive(input.agent_transcript_path ?? input.transcript_path, expectText, Boolean(input.agent_transcript_path)) : null;
    entry = {
      id: crypto.randomUUID(),
      ...meta,
  ...
  try {
    ...
    if (e.message?.includes("permission denied") || e.message?.includes("does not exist")) {
      log7("table missing, creating and retrying");
...
function maybeTriggerPeriodicSummary(sessionId, cwd, config) {
  ...
  try {
    ...
    if (!tryAcquireLock(sessionId)) {
      log7(`periodic trigger suppressed (lock held) session=${sessionId}`);
      return;
    }
    ...
    try {
      ...
      log7(`periodic spawn failed: ${e.message}`);
      try {
        releaseLock(sessionId);
      } catch (releaseErr) {
        log7(`releaseLock after periodic spawn failure also failed: ${releaseErr.message}`);
      }
      throw e;
    }
  } catch (e) {
    log7(`periodic trigger error: ${e.message}`);
  }
}
main().catch((e) => {
