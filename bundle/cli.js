...
wiki: ${planned.length - pending.length}/${planned.length} pages generated \u2014 pending: ${pending.map((g) => wikiDocId(g.key)).join(", ")}`);
  ...
  if (sub === "sync") {
    ...
    if (!loadCurrentSnapshot(cwd)) {
      ...
      const { runBuildCommand } = await import("./graph-H46PXVVB.js");
...
async function runInstallAll(args) {
  ...
  await runInstallDocsOnboarding({
    ...
    onboard: async ({ root, orgId, orgName }) => {
      const { runDocsOnboarding: runDocsOnboarding2 } = await import("./onboarding-7647UCQK.js");
...
async function main() {
  ...
  if (cmd === "graph") {
    ...
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-H46PXVVB.js"));
    } catch (err) {
